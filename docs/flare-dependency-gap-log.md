...
### DEV-CHAT-007 更新记录（上下文连续性排查与排期冻结）
1. 本任务新增了哪些 xiaocai 本地资产：
   - 无代码改动；仅完成线上证据排查与修复排期冻结。
2. 本任务暴露了哪些 FLARE 缺口：
   - 本次不新增 FLARE blocker；主要问题位于 xiaocai adapter 的状态承接与策略分流层。
3. 哪些缺口阻塞当前进度：
   - 阻塞“体验修复上线”，但不阻塞 FLARE kernel 通用能力。
4. 哪些缺口可以临时绕过：
   - 通过 xiaocai 侧最小修复（短回复字段承接 + 直接给方案分流）先改善体验（temporary）。
5. 是否需要向 FLARE 发起单独任务：
   - 暂不新增。先在 xiaocai 侧完成最小修复并观察指标；若仍存在跨实例一致性问题，再升级为 FLARE 任务。

### CHAT-CONTEXT-20260519 证据摘要（确认记录）
1. 现象：
   - 用户连续输入“找一下软件供应商 -> crm -> 我需要你直接给我个找供应商的方案”，回复出现重复追问，用户感知“失忆”。
2. 线上证据：
   - 会话主键一致：`sess_c75cadf26f92`。
   - 消息连续落库，非 session 丢失。
3. 根因判定（当前）：
   - 短回复字段承接弱（`crm` 未稳定承接到结构化字段）。
   - 产品层追问门控覆盖“先给方案”语义。
4. 当前决策：
   - 先文档排期与确认记录，后进入最小代码改动。

### CHAT-CONTEXT-20260519 实施前任务清单（冻结）
1. 代码改动前置输出（必须先完成）
   - 改动文件清单（路径/职责/回滚点）。
   - 风险说明（哪些场景可能受影响）。
   - 验收脚本（包含“找供应商 -> crm -> 直接给方案”）。
...
3. 本次不做事项（明确排除）
   - 不重写 session/runtime/transport。
   - 不迁移 FLARE kernel 逻辑到 xiaocai。
   - 不做大规模 prompt 重构。

### CHAT-NAV-20260519 任务排序（文档先行）
1. 定位结论：
   - “回答+导航双通道”属于 xiaocai 产品层优化，不是 FLARE blocker。
2. P0（先做）：
   - 定义导航通道最小 contract（阶段、进度、已确认、待确认、下一步、v0状态）。
   - 定义不覆盖回答通道的约束与回滚点。
3. P1（后做）：
   - 将导航触发条件与优先级参数化（domain-pack / 策略配置）。
   - 增加灰度指标：重复追问率、阶段可见率、v0产出率。
4. P2（再做）：
   - 在 analysis/sourcing/artifact 场景统一导航口径。
5. FLARE 任务判定：
   - 当前不新增 FLARE handoff task；先在 xiaocai 内验证策略有效性。
