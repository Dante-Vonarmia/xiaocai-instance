"""
会话存储 - Session/Message 管理

职责:
1. 管理用户会话列表
2. 管理会话消息历史
3. 提供按 project/user 过滤能力
"""
...
class ConversationStore:
    ...
    def _init_schema(self) -> None:
        ...
        if self._runtime.backend == "sqlite":
            columns = self._runtime.fetchall("PRAGMA table_info(sessions)")
        ...
        async with self._lock:
            ...
            if self._runtime.backend == "postgres":
                self._runtime.execute(
                    ...
                    (session_id, function_type, project_id, user_id, mode, title, status, preview, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    ...
    async def append_message(
        ...
        async with self._lock:
            ...
            self._runtime.execute(
                """
                INSERT INTO messages (message_id, session_id, user_id, role, content, created_at)
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                (
                    message.message_id,
            ...
            self._runtime.execute(
                ...
                WHERE session_id = ? AND user_id = ?
                """,
                (content[:80], message.created_at, session_id, user_id),
            )
            self._runtime.commit()
            return message
    ...
    async def append_exchange(
        ...
        async with self._lock:
            ...
            self._runtime.execute(
                ...
                VALUES (?, ?, ?, ?, ?, ?)
                """,
                (_new_id("msg_user"), session_id, user_id, "user", user_message, now),
            )
            ...
            self._runtime.execute(
                ...
                WHERE session_id = ? AND user_id = ?
                """,
                (assistant_message[:80], updated_at, session_id, user_id),
            )
            self._runtime.commit()
            return True
